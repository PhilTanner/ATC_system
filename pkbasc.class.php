...
LIMIT 1;';
	...
	-- Jack Bridge. Miscount of $20. Caused because he started year with $20 outstanding from 2012, not entered anywhere in system. Presumption of zero based count fails. Need to reduce his balance by $20. Then make negative payment for 2012 history payment.
	
	-- Hermione Cook. Same deal. Opening balance of 43 (in credit) needs to be accounted for.
	
	-- Ben Duffy, non problem - issue with payment dates in balance inclusions
	
	-- Hary Hall - Send invoice, should start at -7.50, is starting at -20.  Resolution for Harry Hall = zero balance. Paid up to date.
	
	-- Lucas' No action necessary. Discrepency due to mis-calc of payments vs invoice dates. All correct when accoutned for

	-- Macindoe Max Credit from 2012 = $8.5  
	
	-- Martin Ben - System count, probably right. Error in balance table
	
	-- Rockell x 2 - No action necessary. Discrepency due to mis-calc of payments vs invoice dates. All correct when accoutned for
	
	-- Stewart Holly. Had $150 in credit at end of 2012. All balances at this point.
	
	-- Sullivan Olivia.  Account FUBAR'd. Zero balance as of 21st April. Write off as fuckup.
	
	-- Tierney x2 - Credit from 2012 - $28.25
	
	Tollemache, Aidan - Delete last invoice. Zero account to zero balance. Regenerae last invoice. Problem resolved
	
	-- Wratt Ethan.  Account FUBAR'd. Zero balance as of 21st April. Write off as fuckup.
	
	-- Ymker, x 2 zero balance as of 19 Apr
	
	
	-- Wratt & Sullivan need to investigate why balance right on 19 Apr, but not when last invoice.
	
	
	******* What happens to "Total chargese this invoice" when account is in credit?? ******** 
	******* Cannot invoice for lines added after end of previous invoice (Aiden Tollenmarche - 18th Feb records added after end of previous invoice finish) ********
	-- ******* start date defaults to the last invoice date.... *****

	UPDATE `payments` SET invoiced_id = 0 WHERE `date_received` <= '2013-04-28' AND `benefit_payment` = 0;
	
	-- Jack Bridge
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 28, 0, 20, 'Carry over from 2012', '2012-12-25', 0 );
	-- Hermione Cook
	UPDATE student SET balance=-42.5 WHERE student_id=16 LIMIT 1;
	-- Harry Hall
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 30, 0, -12.5, 'Adjustment for zero balance', '2012-12-25', 0 );
	-- Macindoe Max
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 21, 0, 8.5, 'Carry over from 2012', '2012-12-25', 0 );
	-- Martin Ben
	UPDATE student SET balance=22.80 WHERE student_id=9 LIMIT 1;
	-- Stewart Holly
	UPDATE student SET balance=125 WHERE student_id=6 LIMIT 1;
	-- Sullivan Olivia
	DELETE FROM `payments` WHERE student_id=32 AND amount=145 AND benefit_payment=0 AND date_received='2013-04-16 00:00:00' LIMIT 1;
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 32, 0, 145, 'Zero account', '2012-12-25', 0 );
	UPDATE student SET balance=0 WHERE student_id =32 LIMIT 1; 
	-- Tierney x2
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 33, 0, 28.25, 'Carry over from 2012', '2012-12-25', 0 );
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 34, 0, 28.25, 'Carry over from 2012', '2012-12-25', 0 );
	-- Wratt Ethan
	UPDATE student SET balance=0 WHERE student_id=35;
	-- Ymker x2
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 36, 0, -18, 'Adjustment for zero balance for 2013-04-19', '2012-12-25', 0 );
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 37, 0, -18, 'Adjustment for zero balance for 2013-04-19', '2012-12-25', 0 );
	UPDATE student SET balance=0 WHERE student_id IN (36,37);
	
	
	
	New list.
	
	-- Ben Martin should be zero. Paid the last invoice. Owed 50.20. Paid 50.50 (18th, 24th Apr.)
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 9, 0, 12.5, 'Adjustment for balance', '2012-12-25', 0 );
	INSERT INTO `payments` (student_id, invoiced_id, amount, reference, date_received, benefit_payment ) VALUES ( 9, 0, -35, 'Adjustment for balance', '2012-12-25', 0 );
	UPDATE `payments` SET invoiced_id = NULL WHERE `payment_id` IN (141,147);
	
	
	Ethan Wratt - last invoice wasn't complete - WINS back payment came in after it - he's correct. $25 credit
	
	New invoices need to have invoiced amount at last date, plus a both, 
